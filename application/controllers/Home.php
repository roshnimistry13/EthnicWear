...
class Home extends CI_Controller
{
	...
	/***** SEND EMAIL FOR RESET PASSWORD LINK  FROM FORGOT PASSWORD FORM */
	function forgotPassword(){
		$json = array();
		if($this->input->is_ajax_request()){
			$email 				= $this->input->post('txt_cust_email');			
			$data['email']		= $email;
			$res 				= $this->Master_m->where('customer_detail',$data);
			
			if(!empty($res)) {
				$customer_name 		= $res[0]['customer_name']; 
				$customer_id 		= $res[0]['customer_id']; 
				$current_dt 		= date('Y-m-d H:i:s');
				$current_timestamp 	= strtotime($current_dt);
				$email_html 		= '<div style="padding:5% 0%" align="center">
											...
											<tbody>
												<tr>
													<td valign="top" align="center">
														<table style="background-color:#eee;color:#000;font-family:sans-serif; border-radius:3px 3px 0 0"
															...
															<tbody>
																<tr>
																	<td style="padding:15px 20px; display:block">
																		<h3 style="font-size:20px;font-weight:700;line-height:100%;margin:0;text-align:left;color:#000"">
																		Reset Your Password
																		</h3>
																	</td>
																</tr>
												...
												<tr>
													<td style="padding:20px 20px 0px 20px;">
														<div style="font-size: 14px; display: block; letter-spacing: 1px; color: #868686; font-weight: normal;">
															<h3>Hi '.ucwords($customer_name).',</h3>
														</div>
														<div style="font-size: 14px; display: block; letter-spacing: 1px; color: #868686; font-weight: normal;padding-bottom:10px;">
															<a href="'.base_url('reset-password?temp_id=').$customer_id.'&t='.$current_timestamp.'" class="btn">Click here to reset your password</a>
														</div>							
														<div style="font-size: 12px; display: block; letter-spacing: 1px; color: #868686; font-weight: normal;padding-bottom:10px;">
				...
				$mailData['toID'] 				= 'devloperproactii@gmail.com'; //$email;			
				$mailData['message'] 			= $email_html;
				
				$send = send_email($mailData);	
				if($send){
					$json['success'] = "success";
					$json['message'] = "Check your mail inbox to reset password";
				}			
			
			}else{
				$json['error'] = "error";
				$json['message'] = "invalid email id! try again.";
			}
		}
		$this->output->set_content_type('application/json', 'utf-8');
		$this->output->set_output(json_encode($json));
	...
	public function loadProducts($rowno){
		...
		foreach($products as $product){
			...
			if(!empty($variant_list)){
				...
				$elearr[$i]['rate'] 		=  $this->Master_m->getAverageStarrating($product_id);	
			}
			$i++;
		}
		...
		if($this->input->is_ajax_request()){
			if(!empty($this->session->userdata[CUSTOMER_SESSION])){
				...
				if($cart){
					...
					$cart_html 			= $this->Master_m->createMiniCartList($cart_data); 
		...
		if($this->input->is_ajax_request()){
			...
			if($checkitemStock == 0){
				...
				if($result){
					...
					$json['redirect'] 	= base_url('my-account');
	...
	public function restPassword(){
		...
		$link_expire_min 	= LINK_EXPIRE_MIN;  // constant.php	
		$diff_min 			= 0;
		if($timeStamp != "" || $timeStamp != null){
			$time_to_date 		= date('Y-m-d H:i:s', $timeStamp);
			$first_date 		= new DateTime($time_to_date);
			$second_date 		= new DateTime();
			$difference 		= $first_date->diff($second_date);
			$diff_min 			= $difference->i;			
		}
		
		if($diff_min <= $link_expire_min){			
			$meta_data['meta_title']			= "Reset Password | ".UI_THEME;
			$meta_data['meta_description']		= "Reset Password | ".UI_THEME;
			$meta_data['meta_keyword']			= "Reset Password | ".UI_THEME;
			$meta_data['active_menu']			= "Reset Password";
			
			$this->load->view('UI/Common/Header',$meta_data);
			$this->load->view('UI/ResetPassword_v');
			$this->load->view('UI/Common/Footer');
		}
		else{
			$this->load->view('UI/LinkExpire_v');
		}	
		
	}

	/*** RESET PASSWORD */
	public function submitRestPassword(){
		$json = array();
		if($this->input->is_ajax_request()){
			$new_password 		= $this->input->post('txt_cust_password');
			$confrim_password 	= $this->input->post('txt_confirm_password');
			$customer_id 		= $this->input->get('temp_id');
			...
			if($new_password == $confrim_password){
				...
				$json['error'] = "Enter Password Again";
			}			
		}
		$this->output->set_content_type('application/json', 'utf-8');
		$this->output->set_output(json_encode($json));
