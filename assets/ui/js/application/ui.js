...
function createProductGrid(result, sno, whishList) {
	...
	for (index in result) {
		...
		sno += 1;
		var wish_class 					= "";
		var wish_lable 					= "Wishlist";
		var wish_icon 					= "fi-rs-heart";
		var mrp 						= "";
		var product_image 				= result[index].cover_img;
		var product_detail_url 			= base_url + "product-detail/" + short_code;
		if (product_image) {
			
			var img_arr 				= result[index].image.split("|");
			var hover_img_url 			= base_url + PRODUCT_IMAGE_PATH + id + "/" + img_arr[1];
		...
		if (whishList.length > 0) {
			if ($.inArray(id, whishList) != -1) {
				wish_class 		= "wishlist-add";
				wish_lable 		= "Wishlisted";
				wish_icon 		= "fa fa-heart";
			}
		}
		...
		var star_width     = "width:0%";
		if(rate != "" || rate != null || rate != undefined){
			if(rate == 1) star_width     = "width:20%";
			else if(rate == 2) star_width     = "width:40%";
			else if(rate == 3) star_width     = "width:60%";
			else if(rate == 4) star_width     = "width:80%";
			else if(rate == 5) star_width     = "width:100%";
		}
		...
		if (variants != undefined) {
		 ...
		 hover_img_url +
		 '" alt=""></a></div><div class="product-badges product-badges-position product-badges-mrg"><span class="hot">Hot</span></div></div><div class="product-content-wrap mt-20"><h2 class="truncate"><a href="'+product_detail_url+'">'+product_name+'</a></h2><div class="product-rate-cover"><div class="product-rate d-inline-block"><div class="product-rating" style="'+star_width+'"></div></div></div><div class="product-price"><span>₹'+moneyformate(price)+' </span>'+mrp+'</div><div class="product-action-1 show"><a aria-label="'+wish_lable+'" class="action-btn hover-up '+ wish_class +'" href="javascript:void(0)" data-pid="'+ id +'" onclick="additemToWishlost('+ id +')"><i class="'+wish_icon+'"></i></a></div></div></div></div>';
...
$("form[id='forgot_password_form']").validate({
	...
	// Specify validation error messages
	messages: {
		txt_cust_email: {required : "Please enter a email" },
	},
	submitHandler: function (form) {
		$.ajax({
			url: $(form).attr("action"),
...
$("form[id='reset_password']").validate({
	// Specify validation rules
	rules: {
		txt_cust_password: {
			required: true,
		...
		txt_confirm_password: {
			required: true,
			equalTo: "#txt_cust_password",
		},
	},
	// Specify validation error messages
	messages: {
		txt_cust_password: "Please enter password",
		txt_confirm_password: { required: "Please confrim password" },
	},
	submitHandler: function (form) {
		// form.submit();
		$.ajax({
