(function ($) {
    ...
    $(function() {
        ...
        amountprice.val("₹" + sliderrange.slider("values", 0) +
            ...
            $('#min_price').val(sliderrange.slider("values", 0));
			$('#max_price').val(sliderrange.slider("values", 1));
    });
   ...
   /*---------------------
    ...
    $("body").on("click", ".rating_sp_kl", function (evt) {
        ...
        if ($this.attr("href") === "#Reviews") {
            evt.preventDefault();
            evt.stopPropagation();
            let tab$ = $('.ul_tabs a[href="#Reviews"]');
            if (tab$.length) {
                $('.ul_tabs .nav-link').removeClass('active');
                tab$.addClass('active');
    ...
    $.ajax(
        {
            ...
            success: function (data)
            {
				...
				{	
					...
					$('.subcat_tag').remove();
					var subcat_filter = '<a href="javascript:void(0)" class="text-muted clear-filter subcat_tag"> <i class="fi-rs-cross-small"></i> '+data.category_name+'</a>';					
					$('.totall-product').append(subcat_filter);
				}
